...
export default function PlaybackMethodsCard() {
  ...
  const getTranscodeBubbles = (session: SessionDetail) => {
    ...
    if (session.video_method === 'Transcode') {
      bubbles.push(
        <span key="video" className="px-2 py-1 bg-orange-500/20 text-orange-400 border border-orange-400/30 rounded text-xs">
          Video
        </span>
      );
    }
    ...
    if (session.audio_method === 'Transcode') {
      bubbles.push(
        <span key="audio" className="px-2 py-1 bg-orange-500/20 text-orange-400 border border-orange-400/30 rounded text-xs">
          Audio
        </span>
      );
    }
          ...
          <div>
            ...
            <div className="space-y-2 max-h-96 overflow-y-auto">
              {filteredSessions.map((session, idx) => (
                <div key={`${session.session_id}-${idx}`} className="py-3 px-4 bg-neutral-700/30 rounded hover:bg-neutral-700/50 transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-gray-400 space-y-1">
                        <div className="flex flex-wrap gap-2">
                          ...
                          <span>•</span>
                          <span>{session.client_name || session.device_name || session.device_id}</span>
                        </div>
