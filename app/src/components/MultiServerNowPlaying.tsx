...
const colorByType: Record<string, { ring: string; badge: string; text: string }> = {
  ...
  jellyfin: {
    ring: "ring-[#aa5cc8]",
    badge: "bg-[#aa5cc8]/20 text-[#aa5cc8]",
    text: "text-[#aa5cc8]",
  },
};
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {sessions.map((s) => {
          ...
          return (
            <div
              key={s.session_id}
              className={`relative rounded-2xl border border-neutral-800 bg-neutral-900/60 p-3 ring-2 ${col.ring}`}
            >
              <div className="absolute top-2 right-2 text-xs px-2 py-0.5 rounded-full border border-neutral-700 bg-neutral-800 text-gray-300">
                <span
                  className={`mr-1 inline-block w-2 h-2 rounded-full ${col.text.replace("text-", "bg-")}`}
                ></span>
              ...
              <div className="flex gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-2 h-2 bg-neutral-800 rounded-full overflow-hidden">
                    <div
                      className={`h-full ${col.text.replace("text-", "bg-")}`}
                      style={{ width: `${pct}%` }}
                    />
                  </div>
                  <div className="mt-1 text-xs text-gray-400">
                    {s.position_sec ?? 0}s / {s.duration_sec ?? 0}s ·{" "}
                    {((s.bitrate || 0) / 1_000_000).toFixed(1)} Mbps
                  </div>
                  {s.play_method?.toLowerCase() === "transcode" && (
                    <div
                      className={`inline-block mt-2 text-[11px] px-2 py-0.5 rounded ${col.badge} border border-transparent`}
                    >
                      {s.stream_detail || s.stream_path || "Transcode"}
                    </div>
                  )}
                </div>
              </div>
        ...
        {sessions.length === 0 && (
          <div className="text-sm text-gray-400">
            No active sessions {server !== "all" ? `for ${server}` : "now"}.
          </div>
        )}
      </div>
    </section>
