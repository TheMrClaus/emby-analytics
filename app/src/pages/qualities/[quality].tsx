...
export default function QualityDetailPage() {
  ...
  const { quality, media_type } = router.query;
  const [data, setData] = useState<ItemsByQualityResponse | null>(null);
  ...
  if (loading) {
    return (
      <>
        <Head>
          <title>Loading Quality Details - Emby Analytics</title>
        </Head>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-400">
                {fmtInt(data.total)} items found • Resolution range: {data.height_range}
              </p>
            </div>
          </div>

          {/* Items Table */}
          <Card title={`${data.quality} quality items`}>
            {data.items.length === 0 ? (
              <div className="text-center py-8 text-gray-400">
                No items found for this quality level
              </div>
            ) : (
              <>
