...
const endpoints: Endpoint[] = [
  ...
  { id: 'admin-backfill-started-at', category: 'Admin', method: 'POST', path: '/admin/cleanup/backfill-started-at', description: 'Backfill started_at from earliest events or intervals (preserve if earlier).', usage: 'Fix sessions showing incorrect start times due to reactivation overwrites.', dangerous: true },
  { id: 'admin-remap-item-dry', category: 'Admin', method: 'GET', path: '/admin/remap-item', description: 'Dry-run: show references for remapping from one item_id to another.', usage: 'Migrate history from old ID to new ID.', params: [{ key: 'from_id', kind: 'query', required: true, placeholder: 'old_id' }, { key: 'to_id', kind: 'query', required: true, placeholder: 'new_id' }] },
  { id: 'admin-remap-item-apply', category: 'Admin', method: 'POST', path: '/admin/remap-item', description: 'Apply: remap play_intervals and play_sessions to new item_id; delete old library_item.', usage: 'Fix broken links while preserving history.', params: [{ key: 'from_id', kind: 'body', required: true, placeholder: 'old_id' }, { key: 'to_id', kind: 'body', required: true, placeholder: 'new_id' }] },
  { id: 'admin-cleanup-jobs', category: 'Admin', method: 'GET', path: '/admin/cleanup/jobs', description: 'List recent cleanup job audit logs.', usage: 'View cleanup operation history.', params: [{ key: 'limit', kind: 'query', placeholder: '50' }] },
  { id: 'admin-cleanup-job-details', category: 'Admin', method: 'GET', path: '/admin/cleanup/jobs/:jobId', description: 'View detailed audit log for specific cleanup job.', usage: 'Review what items were processed in a cleanup operation.', params: [{ key: 'jobId', kind: 'path', required: true, placeholder: 'job-uuid' }] },
  { id: 'admin-cleanup-scheduler-stats', category: 'Admin', method: 'GET', path: '/admin/cleanup/scheduler/stats', description: 'View cleanup scheduler statistics and next run times.', usage: 'Monitor automated weekly cleanup schedule.' },
  ...
  { id: 'stats-top-users', category: 'Stats', method: 'GET', path: '/stats/top/users', description: 'Top users by watch time.', usage: 'Leaderboard of users.', params: [{ key: 'timeframe', kind: 'query', placeholder: '1d|3d|7d|14d|30d' }, { key: 'limit', kind: 'query', placeholder: '10' }] },
  { id: 'stats-top-items', category: 'Stats', method: 'GET', path: '/stats/top/items', description: 'Top items by watch time (merges live intervals).', usage: 'Find most watched items.', params: [{ key: 'timeframe', kind: 'query', placeholder: '1d|3d|7d|14d|30d' }, { key: 'limit', kind: 'query', placeholder: '10' }] },
  { id: 'stats-top-series', category: 'Stats', method: 'GET', path: '/stats/top/series', description: 'Top series by watch time (across episodes).', usage: 'Find most watched series.', params: [{ key: 'timeframe', kind: 'query', placeholder: '1d|3d|7d|14d|30d' }, { key: 'limit', kind: 'query', placeholder: '10' }] },
  { id: 'stats-qualities', category: 'Stats', method: 'GET', path: '/stats/qualities', description: 'Distribution of media by resolution bucket.', usage: 'Quality breakdown.' },
  ...
  { id: 'admin-refresh-incremental', category: 'Admin', method: 'POST', path: '/admin/refresh/incremental', description: 'Incremental refresh (new/changed).', usage: 'Lightweight maintenance. Protected.' },
  { id: 'admin-refresh-status', category: 'Admin', method: 'GET', path: '/admin/refresh/status', description: 'Current refresh job status.', usage: 'Track progress. Protected.' },
  { id: 'admin-scheduler-stats', category: 'Admin', method: 'GET', path: '/admin/scheduler/stats', description: 'Scheduler last/next runs stats.', usage: 'Monitoring scheduled syncs. Protected.' },

  // Admin - Maintenance & cleanup
  { id: 'admin-reset-all', category: 'Admin', method: 'POST', path: '/admin/reset-all', description: 'Wipe analytics data (library + sessions).', usage: 'Dangerous reset; requires reimport. Protected.', dangerous: true },
  { id: 'admin-reset-lifetime', category: 'Admin', method: 'POST', path: '/admin/reset-lifetime', description: 'Reset lifetime watch counters only.', usage: 'Recompute lifetime from intervals. Protected.', dangerous: true },
  { id: 'admin-fix-pos-units', category: 'Admin', method: 'ALL', path: '/admin/fix-pos-units', description: 'Normalize position tick units.', usage: 'Data hygiene if needed. Protected.' },
  ...
  // Admin - Webhook & users
  { id: 'admin-webhook-stats', category: 'Admin', method: 'GET', path: '/admin/webhook/stats', description: 'Webhook endpoint info.', usage: 'Configure Emby webhooks. Protected.' },
  { id: 'admin-users-force-sync', category: 'Admin', method: 'POST', path: '/admin/users/force-sync', description: 'Force a users sync from Emby.', usage: 'Refresh user list. Protected.' },
  { id: 'admin-debug-users', category: 'Admin', method: 'GET', path: '/admin/debug/users', description: 'Debug listing of Emby users.', usage: 'Inspect mapped users. Protected.' },
...
export default function APIExplorerPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return endpoints.filter(e => [e.id, e.category, e.path, e.description, e.usage].some(t => t.toLowerCase().includes(q)));
  }, [filter]);
                ...
                <div className="mt-2 grid grid-cols-2 gap-2">
                  {ep.params!.map(p => (
                    <div key={p.key} className="flex flex-col">
                      <label className="text-xs text-gray-400 mb-1">{p.label || p.key} {p.required && <span className="text-red-400">*</span>}</label>
                      <input
                        onChange={(e) => setState(s => ({ ...s, [ep.id]: { ...(s[ep.id] || {}), inputs: { ...(s[ep.id]?.inputs || {}), [p.key]: e.target.value } } }))}
