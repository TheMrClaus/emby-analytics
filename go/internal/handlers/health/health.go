...
func Health(db *sql.DB) fiber.Handler {
	return func(c fiber.Ctx) error {
		...
		if status.Database.OK {
			...
			if dataOK {
				err = db.QueryRow(`SELECT COUNT(*) FROM play_sessions`).Scan(&status.DataIntegrity.SessionCount)
				if err != nil {
					dataOK = false
			...
			if dataOK {
				...
				err = db.QueryRow(`SELECT MAX(started_at) FROM play_sessions WHERE started_at IS NOT NULL`).Scan(&lastSession)
				if err != nil {
					dataOK = false
