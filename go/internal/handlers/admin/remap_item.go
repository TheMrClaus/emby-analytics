    ...
    return func(c fiber.Ctx) error {
        ...
        if apply {
            ...
            if res, err := tx.Exec(`UPDATE play_intervals SET item_id = ? WHERE item_id = ?`, req.ToID, req.FromID); err == nil {
                if n, _ := res.RowsAffected(); n > 0 { updatedIv = int(n) }
            } else { _ = tx.Rollback(); return c.Status(500).JSON(fiber.Map{"error": err.Error()}) }
