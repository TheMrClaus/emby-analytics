...
func Overview(db *sql.DB) fiber.Handler {
	return func(c fiber.Ctx) error {
		...
		err = db.QueryRow(`SELECT COUNT(*) FROM library_item WHERE media_type NOT IN ('TvChannel', 'LiveTv', 'Channel')`).Scan(&data.TotalItems)
		if err != nil {
		...
		err = db.QueryRow(`SELECT COUNT(*) FROM play_sessions WHERE started_at IS NOT NULL`).Scan(&data.TotalPlays)
		if err != nil {
		...
		err = db.QueryRow(`SELECT COUNT(DISTINCT item_id) FROM play_sessions WHERE started_at IS NOT NULL`).Scan(&data.UniquePlays)
		if err != nil {
